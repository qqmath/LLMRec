...
def LLM_request(toy_item_attribute, indices, model_type, augmented_attribute_dict, error_cnt):
    ...
    try: 
        ...
        for i, row in enumerate(rows):
            ...
            country = elements[1]
            language = elements[2]
            augmented_attribute_dict[indices[i]] = {0: director, 1: country, 2: language}
        # pickle.dump(augmented_sample_dict, open('augmented_sample_dict','wb'))
        pickle.dump(augmented_attribute_dict, open(file_path + 'augmented_attribute_dict','wb'))
        ...
        print("ValueError error occurred while parsing the response:", ve)
        # time.sleep(25)
        error_cnt += 1
        if error_cnt==5:
            return 1
        ...
        print("KeyError error occurred while accessing the response:", ke)
        # time.sleep(25)
        error_cnt += 1
        if error_cnt==5:
            return 1
        ...
        print("IndexError error occurred while accessing the response:", ke)
        # time.sleep(25)
        # error_cnt += 1
        # if error_cnt==5:
        #     return 1
